 ...
 class SKMOBILEAPP_BOL_Service
{
     ...
     * Location api url
     */
    const LOCATION_API_URL = 'https://maps.googleapis.com/maps/api/place/';
    ...
    public function getLocationAutocomplete($query)
    {
        ...
        if ($apiKey) {
            ...
            $result = UTIL_HttpClient::get(self::LOCATION_API_URL . 'autocomplete/json', $clientParams);
   ...
   /**
    ...
    public function getLocationDetails($location)
    {
        try
        {
            if ( OW::getPluginManager()->getPlugin('googlelocation')->isActive() )
            {
                $apiKey = OW::getConfig()->getValue('skmobileapp', 'google_map_api_key');
                if ( $apiKey )
                {
                    ...
                    if ( $result && $result->getBody() )
                    {
                        ...
                        if ( $response['status'] == 'OK' )
                        {
                            ...
                            {
                                $responsePrediction = array_shift($response['predictions']);
                                if ( !empty($responsePrediction['description']) && !empty($responsePrediction['place_id']) )
                                {
                            ...
                            if ( $placeId )
                            {
                                ...
                                $result = UTIL_HttpClient::get(self::LOCATION_API_URL . 'details/json', $clientParams);
