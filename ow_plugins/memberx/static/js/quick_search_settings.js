...
var MEMBERX_QuickSearchModel = function( $items, positions )
{
    this.positions = {
        'position1' : undefined,
        'position2' : undefined,
...
MEMBERX_QuickSearchModel.prototype = {
    ...
    addItem : function ( $key , $value, $position )
    {
        ...
        OW.trigger('memberx.quick_search_model_render', self);
    },
    ...
    deleteItem : function ( key, notRenderModel )
    {
        ...
        OW.trigger('memberx.quick_search_model_delete_item', {key: key, value: itemValue});
...
var MEMBERX_QuickSearchView = function( responderUrl )
{
    var self = this;
    ...
    {
        OW.bind('memberx.quick_search_model_render', function() {
            self.renderModel( $model );
            self.saveModel($model);
        } );

        OW.bind('memberx.quick_search_add_item_to_position', function(params) {
        ...
        $('.ow_quicksearch_layout  .ow_quicksearch_position').droppable( {
            ...
            drop: function( event, ui ) {
                if ( ui.draggable )
                {
                    ...
                    {
                        OW.trigger('memberx.on_item_drop_to_position', {positionNode: this, item: ui.draggable });
                    }
                    else
                    {
...
var MEMBERX_ListModel = function( $items, $template )
{
    this.items = {};
    this.construct($items, $template);
}

MEMBERX_ListModel.prototype = {
    ...
    addItem : function ( $key , $value )
    {
       if ( $key && $value )
       {
            var self = this;
            self.items[$key] = $value;
            OW.trigger('memberx.list_model_render',self);
       }
    },
...
var MEMBERX_ListView = function()
{
    ...
    this.init = function( $model, $avalableItemsList )
    {
        ...
        OW.bind('memberx.list_model_render', function() { 
            self.renderModel( $model );
        } );

        OW.bind('memberx.quick_search_model_delete_item', function(params) {
                $model.addItem( params.key, params.value );
        });

        OW.bind('memberx.on_item_drop_to_position', function(params) {
            if ( params.item && params.positionNode )
            {
                ...
                $model.deleteItem(question_name);
                OW.trigger('memberx.quick_search_add_item_to_position', {'key': question_name, 'value': value, 'position': position });
            }
        });
