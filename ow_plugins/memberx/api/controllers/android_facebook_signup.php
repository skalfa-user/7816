...
class MEMBERX_ACTRL_AndroidFacebookSignup extends SKANDROID_ACTRL_FacebookSignUp{
    
    
    public function getFacebookLoginQuestion($params) {
        parent::getFacebookLoginQuestion($params);
    ...
    public function saveFacebookLogin($questionParams) {
        
        $params = json_decode($questionParams['data'], true);
        $accountTypeRestric = MEMBERX_CMP_SearchResultSetting::getString(MEMBERX_CMP_SearchResultSetting::ACCOUNT_TYPE_RESTRICT);
        if ($accountTypeRestric !== 'yes'){
        	parent::saveFacebookLogin($questionParams);
            return;
        }
        ...
        $questionParams['data'] = json_encode($params);
