<?php
/**
 * CodeMonster WatermarkPro
 *
 * This file is part of the CodeMonster WatermarkPro package | Oxwall Plugin.
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 *
 * @package   WatermarkPro
 * @license   OSCL
 * @copyright Copyright (C) CodeMonster.club,  All rights reserved.
 * @link      https://www.codemonster.club/watermarkpro_oxwall_plugin.html
 */
...
class WatermarkProImage {
     ...
     *
     * @param string        $overlay        An image filename or a WatermarkProImage object
	...
	function overlay($overlay, $position = 'center', $opacity = 1, $x_offset = 0, $y_offset = 0) {
        ...
        // Load overlay image
        if( !($overlay instanceof WatermarkProImage) ) {
            $overlay = new WatermarkProImage($overlay);
        }
        ...
        switch (strtolower($position)) {
            case 'top left':
				$x_offset = $offset1; $y_offset = $offset1;	
                $x = 0 + $x_offset;
                $y = 0 + $y_offset;
                break;
            case 'top right':
        		$x_offset = $offset2; $y_offset = $offset1;
            ...
            case 'top':
                ...
                break;
            case 'bottom left':
        		$x_offset = $offset1; $y_offset = $offset2;
                $x = 0 + $x_offset;
                $y = $this->height - $overlay->height + $y_offset;
                break;
            case 'bottom right':
        		$x_offset = $offset2; $y_offset = $offset2;
                $x = $this->width - $overlay->width + $x_offset;
                $y = $this->height - $overlay->height + $y_offset;
                break;
            case 'bottom':
